...
export class FlowEditorEsbComponent implements OnInit, OnDestroy {
  ...
  load(id, isCloning?: boolean): void {
    forkJoin([
    ...
    ]).subscribe(([wikiDocUrl, camelDocUrl, routes, gateways, allendpoints, gatewayName]) => {
      ...
      if (id) {
        ...
        setTimeout(() => {
          ...
          const formgroupRoute = this.initializeEndpointData(this.endpoint);
          (<FormArray>this.editFlowForm.controls.endpointsData).push(formgroupRoute);
          ...
          const formgroupError = this.initializeEndpointData(this.endpoint);
          (<FormArray>this.editFlowForm.controls.endpointsData).push(formgroupError);
  ...
  setPopoverMessages(): void {
    ...
    this.logLevelPopoverMessage = `Sets the log level (default=OFF).`;
    this.errorHandlerPopoverMessage = `Service that handles errors in case of failures.`;
