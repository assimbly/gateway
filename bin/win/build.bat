...
if [%1]==[] (
    ...
    if [%2]==[] (
        cls & ..\..\gradlew clean bootJar --no-daemon -Dorg.gradle.jvmargs="-Xmx4096m -XX:MaxMetaspaceSize=1024m" --settings-file ./../../settings.gradle -Papi-docs -P%1
    ) else (
        if [%3]==[] (
            cls & ..\..\gradlew clean bootJar --no-daemon -Dorg.gradle.jvmargs="-Xmx4096m -XX:MaxMetaspaceSize=1024m" --settings-file ./../../settings.gradle -Papi-docs -P%1 -P%2
        ) else (
            cls & ..\..\gradlew clean bootJar --no-daemon -Dorg.gradle.jvmargs="-Xmx4096m -XX:MaxMetaspaceSize=1024m" --settings-file ./../../settings.gradle -Papi-docs -P%1 -P%2 -P%3
        )
    )
)
